...
no_description = []
puts "Usernames and Descriptions:"
twitter_data.each do |account|
    account.each do |user, data|
        if data["description"] == nil
            no_description.push(user)
            data["description"] = "N/A"
        end
        puts "#{user}: #{data["description"]}"
    end
end
puts "\n"
...
max_follower_user = ''
max_follower_count = -1
puts "Usernames and Follower Counts:"
twitter_data.each do |account|
    account.each do |user, data|
        puts "#{user}: #{data["number of followers"]}"
        if data["number of followers"] > max_follower_count
            max_follower_count = data["number of followers"]
            max_follower_user = user
        end
    end
end
puts twitter_data[1].first.first["last twenty tweets"]
puts "\n"

###### Usernames and Length of Latest Tweet ######
puts "Usernames and Length of Latest Tweet"
twitter_data.each do |account|
    account.each do |user, data|
        puts "#{user}'s latest tweet was #{data["latest tweet"].length} characters"
    end
end
puts "\n"

###### Username and Total Character Count ######
puts "Username and Total Character Count"
twitter_data.each do |account|
    account.each do |user, data|
        total = 0
        ...
        puts "#{user} used #{total} characters in their latest 20 tweets."
    end
end
puts "\n"
...
puts "#{max_follower_user} has the most followers."
puts "\n"
...
twitter_data.each do |account|
    account.each do |user, data|
        if data["number of friends"] > max_friend_count
            max_friend_count = data["number of friends"]
            max_friend_user = user
        end
    end
end
puts "#{max_friend_user} has the most friends."
puts "\n"
...
twitter_data.each do |account|
    account.each do |user, data|
        if data["number of tweets"] > max_tweet_count
            max_tweet_count = data["number of tweets"]
            max_tweeter = user
        end
    end
end
puts "#{max_tweeter} is the most frequent tweeter."
puts "\n"

# Which users have no description?
no_description.each do |user|
    puts "#{user} does not have a description."
end
puts "\n"

# Print out those with location and those without
has_location = []
no_location = []
twitter_data.each do |account|
    account.each do |user, data|
        if data["location"] == nil
            no_location.push(user)
        else
            has_location.push(user)
        end
    end
end

puts "Has a location:"
has_location.each do |user|
    puts user
end
puts "\n"
